...
Credly source adapter in growthfolio inspected (config, section, card, route, types). Its API URL returns 404 and fallback invents issuedAt. New adapter uses official public metadata, extracts only validated fields, leaves unavailable dates null and retains last valid snapshot per badge. No API response recipient fields are stored.
