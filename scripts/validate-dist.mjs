...
for(const path of htmls){
 ...
 if(path==='index.html'||path==='en/index.html'){
  for(const id of ['work','journey','education','contact','open-speech-bridge','database-radar','rizoma','legacy-flight-recorder']) assert.ok(html.includes(`id="${id}"`),`Missing ${id}`);
  const ids=new Set([...html.matchAll(/id="([^"]+)"/g)].map(x=>x[1]));
  for(const [,id] of html.matchAll(/href="#([^"]+)"/g))assert.ok(ids.has(id),`Broken anchor ${id}`);
...
let scriptTotal=0;let cssTotal=0;
for(const f of await readdir('dist/_astro')){
 const buf=await readFile(`dist/_astro/${f}`);const size=gzipSync(buf).length;
...
console.log(`Static routes, anchors, assets, CNAME verified. JS gzip ${scriptTotal}B; CSS gzip ${cssTotal}B.`);
